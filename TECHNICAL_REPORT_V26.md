...
The data and evaluator truth are synthetic, with public-data inputs; generalization to actual bank controls is untested. The local RTX 4090 Laptop GPU and host conditions limit latency transferability. A zero observed unauthorized or duplicate commit count does not prove zero true failure probability. The local ledger requires a trusted external head anchor for tamper evidence. The repository is not a bank system, does not execute real financial actions, and has no production authorization. The repository has no LICENSE file although frozen package metadata declares MIT; publication terms require an owner decision. Future work could test independent organizations, broader policy catalogs, external anchoring, and hardware/operational variability, but those are proposals rather than measured V2.6 results.
