"""Record the V2.5 static and full test admission bar."""
...
def main() -> None:
    ...
    scripts = [str(path.relative_to(ROOT)) for path in sorted((ROOT / "scripts").glob("v25_*.py"))]
    temp = ROOT / "results/v25" / f"pytest_admission_{run_id}"
    jobs = [
        ...
        (
            ...
            [
                str(BIN / "pytest.exe"),
                "-q",
                "--junitxml=results/v25/v25_tests.xml",
    ...
    report = {
        ...
        "run_id": run_id,
        "checks": outcomes,
        "junit_sha256": sha256_file(ROOT / "results/v25/v25_tests.xml")
        if passed and (ROOT / "results/v25/v25_tests.xml").is_file()
        else None,
    }
    atomic_write_json(ROOT / "state/v25_static_quality.json", report)
    print(report["status"])
    if not passed:
        raise SystemExit(1)
