...
if [[ "${V25_RUN_ROLE:-}" != "development" && "${V25_RUN_ROLE:-}" != "qualification" && "${V25_RUN_ROLE:-}" != "final" ]]; then
  echo "V25 run role must be development, qualification, or final" >&2
  exit 72
fi
...
export V23_RUN_ROLE="$V25_RUN_ROLE"
