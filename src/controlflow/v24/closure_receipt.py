...
from controlflow.v24.artifact_closure import binding, unbound_files, verify_bindings


def create_receipt(root: Path, graph_path: Path, output: Path) -> dict[str, Any]:
    graph = json.loads(graph_path.read_text(encoding="utf-8"))
    rules = yaml.safe_load((root / graph["rules"]["path"]).read_text(encoding="utf-8"))
    paths = [root / name for name in rules["state_substantive_files"]]
    receipt: dict[str, Any] = {
        "schema_version": 1,
        "status": "CLOSURE_BOUND",
        "qualification_graph": binding(root, graph_path),
        "deferred_bindings": [binding(root, path) for path in paths],
        "terminal_root_anchors": rules["terminal_root_anchors"],
    }
...
def verify_receipt(root: Path, graph_path: Path, receipt_path: Path) -> dict[str, Any]:
    ...
    rules = yaml.safe_load((root / graph["rules"]["path"]).read_text(encoding="utf-8"))
    declared = set(rules["state_substantive_files"])
    bound = {row["path"] for row in receipt["deferred_bindings"]}
    if declared != bound or receipt.get("terminal_root_anchors") != rules["terminal_root_anchors"]:
        failures.append("state_artifact_inventory_mismatch")
    permitted_state = declared | set(rules["terminal_root_anchors"])
    for pattern in rules["state_substantive_patterns"]:
    ...
    decision = json.loads((root / "results/v24/qualification/gate_decision.json").read_text(encoding="utf-8"))
    postclose = json.loads((root / "state/v24_postclose_verification.json").read_text(encoding="utf-8"))
