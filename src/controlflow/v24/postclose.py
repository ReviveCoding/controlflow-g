...
from controlflow.v24.artifact_closure import unbound_files, verify_bindings
...
def verify(root: Path, graph_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    interval = yaml.safe_load((root / "configs/v24/strata.yaml").read_text(encoding="utf-8"))[
    ...
    try:
        ...
        recalculated = recompute(
            ...
            None,
            critical_threshold=float(payload("checkpoint")["critical_threshold"]),
        )
    ...
    if (
        ...
        or provenance.get("run_role") != "qualification"
        ...
        or "V23_RUN_ROLE=qualification" not in provenance.get("environment", [])
    ...
    if not payload("checkpoint") or not payload("environment_manifest"):
