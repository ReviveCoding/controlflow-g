...
from controlflow.v24.generator import STALE_ALTERNATIVES
...
def structural_admission(
    directory: Path, root: Path, contamination: dict[str, Any], output: Path | None = None
) -> dict[str, Any]:
    failures: list[str] = []
    required = {
    ...
    if not manifest_path.is_file() or any(not path.is_file() for path in required.values()):
        result = {"schema_version": 1, "status": "V24_GENERATION_INVALID", "failures": ["missing_required_file"]}
    ...
    if len(authorization) != 600 or len(set(authorization.identity.astype(str))) != 600:
    ...
    contract = yaml.safe_load((root / "configs/v24/denominator_contract.yaml").read_text(encoding="utf-8"))
    ...
    result = {
        "schema_version": 1,
        "status": "ADMITTED" if not failures else "V24_GENERATION_INVALID",
        "count": len(runtime),
