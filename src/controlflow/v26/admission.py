...
from controlflow.v26.generator import STALE_ALTERNATIVES
...
def structural_admission(
    ...
    *,
    ...
    if not manifest_path.is_file() or any(not path.is_file() for path in required.values()):
        result = {"schema_version": 1, "status": "V26_GENERATION_INVALID", "failures": ["missing_required_file"]}
        if output is not None:
            atomic_write_json(output, result)
        return result
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    strata_file = "rehearsal_strata.yaml" if role == "DEVELOPMENT" else "strata.yaml"
    strata_config = yaml.safe_load((root / "configs/v26" / strata_file).read_text(encoding="utf-8"))
    seed_key = "development_seed" if role == "DEVELOPMENT" else f"{role.lower()}_seed"
    expected_seed = int(strata_config[seed_key])
    if role == "DEVELOPMENT":
        suffix = directory.name.removeprefix("rehearsal_")
        if not directory.name.startswith("rehearsal_") or not suffix.isdecimal():
            raise ValueError("V26_DEVELOPMENT_DIRECTORY_INVALID")
        expected_seed = int(suffix)
    expected_prefix = {
        "DEVELOPMENT": f"V26DEV{expected_seed}",
        "QUALIFICATION": "V26QUAL",
        "FINAL": "V26FINAL",
    }[role]
    if (
        ...
        or manifest.get("generator") != "controlflow.v26.generator.generate_v26_split"
    ...
    contract = yaml.safe_load((root / "configs/v26" / contract_name).read_text(encoding="utf-8"))
    ...
    if contamination.get("semantic", {}).get("leakage_findings") != 0:
    ...
    result = {
        "schema_version": 1,
        "status": "ADMITTED" if not failures else "V26_GENERATION_INVALID",
        "count": len(runtime),
