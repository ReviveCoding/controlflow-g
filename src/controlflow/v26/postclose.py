...
from controlflow.v26.artifact_closure import unbound_files, verify_bindings
...
def _approval_read_only(database: Path) -> dict[str, Any]:
    connection = sqlite3.connect(f"file:{database.as_posix()}?mode=ro&immutable=1", uri=True)
    connection.row_factory = sqlite3.Row
    try:
        foreign_keys = list(connection.execute("PRAGMA foreign_key_check"))
        events = [dict(row) for row in connection.execute("SELECT * FROM action_ledger")]
        consumed = [dict(row) for row in connection.execute("SELECT * FROM approval_consumption")]
    finally:
        connection.close()
    expected = {
        str(row["event_id"]): (str(row["approval_token_id"]), str(row["created_at"]))
        for row in events
        if row["committed"] == 1
        and row["review_required"] == 1
        and row["approval_valid"] == 1
        and row["approval_token_id"] is not None
    }
    actual = {str(row["event_id"]): (str(row["token_id"]), str(row["consumed_at"])) for row in consumed}
    review_commit_count = sum(
        row["committed"] == 1 and row["review_required"] == 1 and row["approval_valid"] == 1 for row in events
    )
    return {
        "valid": not foreign_keys
        and len(expected) == review_commit_count
        and len(actual) == len(consumed)
        and expected == actual,
        "foreign_key_failures": len(foreign_keys),
        "review_commit_count": review_commit_count,
        "approval_consumption_count": len(consumed),
    }
...
def verify(root: Path, graph_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or finalized.get("foreign_key_check") != "ok"
    ...
    interval = yaml.safe_load((root / "configs/v26" / strata_name).read_text(encoding="utf-8"))[
    ...
    approval = _approval_read_only(database)
    if (
        not approval["valid"]
        or finalized.get("approval_consumption_count") != approval["approval_consumption_count"]
        or finalized.get("review_commit_count") != approval["review_commit_count"]
    ):
        failures.append("approval_consumption_invalid")
    ...
    freeze: dict[str, Any] = {}
    try:
        seed = int(str(graph["identity"]).removeprefix("development_rehearsal_")) if role == "development" else None
        freeze = verify_freeze(root, role=role, development_seed=seed)
    ...
    if (
        ...
        or payload("contamination_report").get("semantic", {}).get("leakage_findings") != 0
        or payload("transition_receipt").get("status") != "PASS"
    ):
