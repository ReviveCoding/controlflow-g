...
def rules_for_graph(root: Path, graph: dict[str, Any]) -> dict[str, Any]:
    ...
    rehearsal = identity.removeprefix("development_")
    return {
        key: [item.format(identity=identity, rehearsal=rehearsal) for item in value]
...
def make_graph(
    root: Path, named_paths: dict[str, Path], output: Path, *, role: str, identity: str | None = None
) -> dict[str, Any]:
    ...
    graph = {
        ...
        "rules": binding(root, root / "configs/v26/artifact_rules.yaml"),
    }
