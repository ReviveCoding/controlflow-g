...
from controlflow.eval.metrics import holm_adjust
...
def run() -> str:
    ...
    entities = (
        traces[traces.experiment_id == "ablation-AB12_full_controlflow_g"]
        .set_index("case_id")
        .loc[full.index, "entity_id"]
        .astype(str)
    )
    unique_entities = entities.unique()
    rng = np.random.default_rng(1729)

    def cluster_interval(differences: pd.Series) -> tuple[float, float, float]:
        by_entity = differences.groupby(entities).mean()
        samples = np.asarray(
            [
                by_entity.loc[rng.choice(unique_entities, len(unique_entities), replace=True)].mean()
    ...
    with PhaseRun("P23", paths) as phase:
        for experiment_id, candidate in traces.groupby("experiment_id"):
            ...
            effect, ci_low, ci_high = cluster_interval(full - paired)
        ...
        for name, (without_a, without_b, without_both) in cells.items():
            ...
            effect, ci_low, ci_high = cluster_interval(contrast)
            by_entity = contrast.groupby(entities).mean()
            samples = np.asarray(
                [
                    by_entity.loc[rng.choice(unique_entities, len(unique_entities), replace=True)].mean()
                    for _ in range(2_000)
