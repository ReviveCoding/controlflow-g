...
def run() -> str:
    ...
    trace_target = paths.root / "results/ablation_traces.inprogress.parquet"
    summary_target = paths.root / "results/ablation.inprogress.parquet"
    ...
    with GpuSemaphore(), PhaseRun("P22", paths) as phase:
        for name, changes in REMOVALS.items():
            ...
            workflow = GovernedWorkflow(
                train,
                controls,
                ActionLedger(paths.root / f"artifacts/ablation_{name}_action_ledger_v7.sqlite"),
