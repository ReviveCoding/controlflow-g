...
10. P26 uses `state/final_run.json` to bind the one consumed holdout to the exact freeze hash and deterministic run ID. Resume only that run. Completed case/architecture records are fsynced to `results/final_test_checkpoint.jsonl`; never recompute an existing work ID.
